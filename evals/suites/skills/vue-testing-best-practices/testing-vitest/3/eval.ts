...
const testFileContent = readFileSync(
  join(process.cwd(), "src/components/InputField.test.ts"),
  "utf-8"
);
...
test("Uses @vue/test-utils mount", () => {
  ...
  // Should call mount
  expect(testFileContent).toMatch(/mount\s*\(\s*InputField/);
});

test("Tests component interactions", () => {
  // Should test clicking buttons or setting value
  expect(testFileContent).toMatch(/trigger\s*\(\s*['"]click['"]\s*\)|setValue/);

  // Should use async/await for interactions
  expect(testFileContent).toMatch(/await/);
});
